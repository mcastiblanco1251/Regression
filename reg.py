...
if uploaded_file is not None:
    ...
    def user_input_features():
        ...
        symboling = st.sidebar.slider('Símbolo (symboling)', int(df.symboling.min()), int(df.symboling .max()), int(df.symboling.mean()))
        wheel_base = st.sidebar.slider('Rueda Base (wheel_base)',float(df.wheel_base.min()), float(df.wheel_base.max()), float(df.wheel_base.mean()))
        length = st.sidebar.slider('Longitud (length)', float(df.length.min()), float(df.length.max()), float(df.length.mean()))
        width = st.sidebar.slider('Ancho (width)', float(df.width.min()), float(df.width.max()), float(df.width.mean()))
        height = st.sidebar.slider('Altura (height)', float(df.height.min()), float(df.height.max()), float(df.height.mean()))
        curb_weight=st.sidebar.slider('Peso (curb_weight)', int(df.curb_weight.min()), int(df.curb_weight.max()), int(df.curb_weight.mean()))
        #engine_type=st.sidebar.slider('Tipo de Motor', float(df.engine_type.min()), float(df.engine_type.max()), float(df.engine_type.mean()))
        engine_size=st.sidebar.slider('Cilidrada (engine_size)', int(df.engine_size.min()), int(df.engine_size.max()), int(df.engine_size.mean()))
        ...
        bore=st.sidebar.slider('Pared cilindro Motor (bore)', float(df.bore.min()), float(df.bore.max()), float(df.bore.mean()))
        stroke=st.sidebar.slider('Tiempos (stroke)', float(df.stroke.min()), float(df.stroke.max()), float(df.stroke.mean()))
        compression_ratio=st.sidebar.slider('Razón de Compresión (compression_ratio)', float(df.compression_ratio.min()), float(df.compression_ratio.max()), float(df.compression_ratio.mean()))
        horsepower=st.sidebar.slider('Potencia (horsepower)', int(df.horsepower.min()), int(df.horsepower.max()), int(df.horsepower.mean()))
        peak_rpm=st.sidebar.slider('Max. RPM (peak_rpm)', int(df.peak_rpm.min()), int(df.peak_rpm.max()), int(df.peak_rpm.mean()))
        city_mpg=st.sidebar.slider('Consumo Ciudad mpg (city_mpg)', int(df.city_mpg.min()), int(df.city_mpg.max()), int(df.city_mpg.mean()))
        highway_mpg=st.sidebar.slider('Consumo Carretera mpg (highway_mpg)', int(df.highway_mpg.min()), int(df.highway_mpg.max()), int(df.highway_mpg.mean()))
        cylinder=st.sidebar.slider('Número de Cilindros (cylinder)', int(df.cylinder.min()), int(df.cylinder.max()), int(df.cylinder.mean()))
